...
class DataProcessor(Dataset):
    def __init__(self, images_path, tokenizer, transforms, max_seq_length):
        self.data = []
    ...
    def to_tensor(self, item):
        ...
        image = Image.open(os.path.join(self.images_path, item["img"])).convert("RGB")
...
class FakedditMM2wayProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {}".format(os.path.join(data_dir, "train_10000.tsv")))
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train_10000.tsv"), '"'), "train")
